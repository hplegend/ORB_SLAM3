...
namespace ORB_SLAM3 {
    ...
    void PointCloudMapping::addCoordinateSystem(std::shared_ptr <pcl::visualization::PCLVisualizer> viewer,
        ...
        viewer->addLine<pcl::PointXYZ, pcl::PointXYZ>(pcl_transform(pose, o), pcl_transform(pose, x_axis), 1.0, 0.0,
                                                      0.0, prefix + "_x");
        viewer->addLine<pcl::PointXYZ, pcl::PointXYZ>(pcl_transform(pose, o), pcl_transform(pose, y_axis), 0.0, 1.0,
                                                      0.0, prefix + "_y");
        viewer->addLine<pcl::PointXYZ, pcl::PointXYZ>(pcl_transform(pose, o), pcl_transform(pose, z_axis), 0.0, 0.0,
                                                      1.0, prefix + "_z");
    }
    ...
    void PointCloudMapping::insertKeyFrame(KeyFrame *kf, cv::Mat &color, cv::Mat &depth) {
        ...
        cout << "1 receive a keyframe, id = " << kf->mnId << endl;
    ...
    void PointCloudMapping::insertKeyFrame(KeyFrame *kf, cv::Mat &left, cv::Mat &right, cv::Mat &Q) {
        ...
        cout << "2 receive a keyframe, id = " << kf->mnId << endl;
        unique_lock <mutex> lck(keyframeMutex);
        // 数据
        keyframes.push_back(kf);
    ...
    void PointCloudMapping::insertKeyFrame(KeyFrame *kf, cv::Mat &left, cv::Mat &right, cv::Mat &disp, cv::Mat &Q) {
        mSensor = MappingSensor::STEREO;
        cout << "3 receive a keyframe, id = " << kf->mnId << endl;
        unique_lock <mutex> lck(keyframeMutex);
        // 数据
        keyframes.push_back(kf);
        colorImgs.push_back(left.clone());
    ...
    PointCloudMapping::GetPointCloud(KeyFrame *kf, cv::Mat &color, cv::Mat &depth) {
    ...
    pcl::PointCloud<PointCloudMapping::PointT>::Ptr
    PointCloudMapping::GetPointCloud(KeyFrame *kf, cv::Mat &left, cv::Mat &right, cv::Mat &Q) {
        cv::Mat left_r = left.clone();
        cv::Mat right_r = right.clone();


        cv::imshow("left_r", left_r);
        cv::waitKey(1);
        cv::imshow("right_r", right_r);
        cv::waitKey(1);
        // 视差计算
    ...
    pcl::PointCloud<PointCloudMapping::PointT>::Ptr
    PointCloudMapping::GetPointCloud(KeyFrame *kf, cv::Mat &left, cv::Mat &right, cv::Mat &disp, cv::Mat &Q) {
        ...
        PointCloud::Ptr tmp(new PointCloud());
        // // 选取有效区域作为稠密重建
        for (int m = static_cast<int>(points_image.rows * 0.25);
             m < static_cast<int>(points_image.rows * 0.75); m += 1) {
            for (int n = static_cast<int>(points_image.cols * 0.25);
                 n < static_cast<int>(points_image.cols * 0.75); n += 1) {
